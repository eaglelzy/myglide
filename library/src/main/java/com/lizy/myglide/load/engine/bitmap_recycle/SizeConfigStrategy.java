...
public class SizeConfigStrategy implements LruPoolStrategy {
    ...
    public void put(Bitmap bitmap) {
        ...
        Integer currentSize = sizes.get(key.size);
        sizes.put(key.size, currentSize == null ? 1 : currentSize + 1);
    }
    ...
    public Bitmap get(int width, int height, Bitmap.Config config) {
        ...
        Bitmap result = groupLinkedMap.get(bestKey);
        if (result != null) {
    ...
    private void decrementBitmapOfSize(int size, Bitmap remove) {
        NavigableMap<Integer, Integer> sizes = getSizesForConfig(remove.getConfig());
        Integer current = sizes.get(size);
        if (current == 0) {
            throw new NullPointerException("try to decrement empty size:"
                    + ", size=" + size
                    + ", remove=" + logBitmap(remove)
                    + ", this=" + this);
        }
    ...
    public Bitmap removeLast() {
        Bitmap removed = groupLinkedMap.removeLast();
        if (removed != null) {
